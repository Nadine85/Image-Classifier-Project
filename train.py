...
#----------------------------------------------------------#
#Function to validate on testdata
...
def parse_arguments():
    ...
    parser.add_argument('--data_dir', type = str, default="flowers", help='data directory must be provided!')
    parser.add_argument('--categories', type = str, default="cat_to_name.json", help='category-to-name-mapping must be provided as .json!')
    parser.add_argument('--arch', type = str, default = 'vgg16_bn', help='either use vgg16_bn or densenet')
...
# check every argument and catch no-allowed-data
if (arguments.arch is None):
    arch_type = 'vgg16_bn'
    
if (arguments.arch != 'vgg16_bn') and (arguments.arch != 'densenet'):
    print("Warning: Network must be vgg16_bn or densenet. Setting arch=vgg16_bn to proceed!")
    arguments.arch = 'vgg16_bn'

if(arguments.learning_rate <= 0):
    print("Warning: Learning rate must be larger than zero. Setting learning_rate=0.05 to proceed!")
    arguments.learning_rate = 0.05
    
if(arguments.hidden_units <= 0):
    print("Warning: hidden units must be larger than zero. Setting hidden units= 512 to proceed!")

if(arguments.epochs<=0):
    print("Warning: epochs must be larger than zero. Setting epochs = 1 to proceed!")
